...
{	
	...
	public function scrapeMovieInfo($imdbUrl, $getExtraInfo = true)
	{   $nameExtractor = '/<a.*?href="\/name\/(.*?)["|\/].*?>(.*?)<\/a>/ms';
		...
		$arr['cast'] = $this->match_all_key_value('/itemprop="name">(.*?)<.*?<td class="primary_photo">.*?<span class="itemprop" itemprop="name">(.*?)<\/span>/ms', $this->match('/<table class="cast_list">(.*?)<\/table>/ms', $html, 1));
		...
		if ($arr['poster'] != '' && strpos($arr['poster'], "m.media-amazon.com") > 0) { //Get large and small posters
			$arr['poster'] = preg_replace('/_V1.*?.jpg/ms', "_V1._SY500.jpg", $arr['poster']);
